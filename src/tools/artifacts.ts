 ...
 */
const VALID_TYPES = ['document', 'code', 'decision', 'conversation', 'file'] as const;
type ArtifactType = (typeof VALID_TYPES)[number];

/**
 * Valid priority levels.
 */
const VALID_PRIORITIES = ['core', 'normal', 'reference'] as const;
...
export async function artifactSave(
  ...
  try {
    ...
    if (!VALID_TYPES.includes(artifact_type as ArtifactType)) {
      return {
        success: false,
    ...
    if (auto_chunk && needsChunking(content)) {
      ...
      for (let i = 0; i < chunked.chunks.length; i++) {
        ...
        const chunkArtifact = await ctx.storage.artifacts.create({
          ...
          priority: effectivePriority as Priority,
...
export async function artifactList(
  ...
  try {
    ...
    const artifacts = await ctx.storage.artifacts.list(project_id, {
      type: artifact_type as ArtifactType | undefined,
      limit,
      offset,
    });
 ...
 */
export async function artifactArchived(
  ctx: ToolContext,
  params: {
    project_id: string;
    limit?: number;
