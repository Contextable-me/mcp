...
import { parseExportFromPath, analyzeExport, } from '../import/index.js';
/**
 ...
 */
export async function importAnalyze(_ctx, params) {
    try {
 ...
 */
export async function importSeed(ctx, params) {
    try {
        const { storage } = ctx;
        const { file_path, project_names, create_context_artifacts = true, include_decisions = true, } = params;
        ...
        const selectedProjects = analysis.detectedProjects.filter((p) => project_names.some((name) => name.toLowerCase() === p.suggestedName.toLowerCase()));
        if (selectedProjects.length === 0) {
            return {
                success: false,
                message: `None of the specified projects were found. Available: ${analysis.detectedProjects.map((p) => p.suggestedName).join(', ')}`,
            };
        }
        const projectIds = [];
        let artifactCount = 0;
        ...
        for (const detected of selectedProjects) {
            ...
            if (include_decisions) {
                const projectDecisions = analysis.decisions.filter((d) => detected.conversationIds.includes(d.conversationId));
 ...
 */
async function createOrUpdateArtifact(storage, data) {
    // Try to find existing artifact with same title in project
    const existing = await storage.artifacts.list(data.project_id, {
        limit: 100,
    });
    const existingArtifact = existing.find((a) => a.title.toLowerCase() === data.title.toLowerCase());
    if (existingArtifact) {
 ...
 */
function buildProjectDescription(project, analysis) {
    const parts = [];
 ...
 */
function buildContextContent(project, analysis) {
    const lines = [];
    ...
    const relevantPatterns = analysis.patterns.filter((p) => p.examples.some((ex) => project.keyTopics.some((topic) => ex.toLowerCase().includes(topic.toLowerCase()))));
