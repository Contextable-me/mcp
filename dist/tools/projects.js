 ...
 */
async function findSimilarProjects(storage, name) {
 ...
 */
export async function projectSave(ctx, params) {
    try {
 ...
 */
export async function projectList(ctx, params = {}) {
    try {
 ...
 */
export async function projectResume(ctx, params) {
    try {
        const { project_id, project_name, load_content = false, max_tokens = DEFAULT_MAX_TOKENS, priority_filter, topic_cluster, } = params;
        // Get project
        let project = null;
        if (project_id) {
        ...
        if (topic_cluster) {
            const filtered = filterByTopic(artifacts.map((a) => ({
                title: a.title,
        ...
        const topicClusters = buildTopicClusters(artifacts.map((a) => ({
            title: a.title,
            summary: a.summary ?? undefined,
            tags: a.tags,
        })));
        // Generate living summary
        const livingSummary = generateLivingSummary({ name: project.name, description: project.description }, artifacts.map((a) => ({
            title: a.title,
            summary: a.summary ?? undefined,
            priority: a.priority,
        ...
        const response = {
            ...
            message: `Loaded '${project.name}' with ${artifacts.length} artifact(s)` +
 ...
 */
export async function projectAnalysisGet(ctx, params) {
    ...
    try {
        ...
        if (!(analysis_type in analyses)) {
            return {
                ...
                message: `No '${analysis_type}' analysis found. Available: ${availableTypes.join(', ') || 'none'}. ` +
