...
export function detectSource(files) {
    ...
    if (detectChatGPT(files))
        return 'chatgpt';
    if (detectGemini(files))
        return 'gemini';
    if (detectClaude(files))
        return 'claude';
    return null;
}
/**
 ...
 */
export async function extractZipFromBuffer(buffer) {
 ...
 */
export async function parseExportFromPath(filePath) {
 ...
 * @throws Error if the format is not recognized
 */
export async function parseExportFromBuffer(buffer) {
    ...
    if (!source) {
        throw new Error('Could not detect export format. Supported formats: ChatGPT, Claude, Gemini');
    }
