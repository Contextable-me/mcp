 ...
 */
export function analyzeExport(exp) {
    ...
    const totalWords = exp.conversations.reduce((sum, c) => sum + c.messages.reduce((s, m) => s + m.content.split(/\s+/).length, 0), 0);
...
function extractConversationKeywords(conv) {
...
function extractTopics(exp, convKeywords, topKeywords) {
    ...
    for (const [domain, terms] of Object.entries(TECHNICAL_DOMAINS)) {
        ...
        if (matchingConvs.length >= 2) {
            ...
            clusters.push({
                name: domain,
                keywords: terms.filter(t => matchingConvs.some(cid => convKeywords.get(cid)?.has(t))).slice(0, 10),
...
function detectProjects(exp, topics) {
    const projects = [];
    // Create a project for each significant topic cluster
    for (const topic of topics.slice(0, 5)) {
        if (topic.conversationCount < 2)
            continue;
